...
interface CardProps {
  ...
  onAddChild: (text: string, parentIdentifier: string, type?:string) => void;
  level: number;
...
const Card: React.FC<CardProps> = ({ data, onDelete, onAddChild, level,childrenLength }) => {
...
const [contentLineWidth, setContentLineWidth] = useState<string | number>("auto");

const [contentLineRefLength,setContentLineRefLength] = useState(0);
  // console.log(childrenLength)
  // console.log("width ",contentLineWidth)
  // console.log("children length ",data.children.length)
...
const handleAddChildSubmit = (childText: string) => {
  ...
  setContentLineRefLength(prev => (contentLineRef.current ? prev = contentLineRef.current?.offsetWidth : prev ))
};
...
useEffect(() => {
  ...
  return (
    ...
    <div className="card" >
      {showInput ? (
        ...
        <div className="card-content-line-container" >
          <hr></hr>
          <div className={`card-content ${data.children && data.children.length > 0 ? 'has-children' : ''} ${level === 1 ? 'level-1' : ''} ${level === 2 ? 'level-2' : ''} ${level === 3 ? 'level-3' : ''}`}>
            <p>{inputValue}</p></div>
          ...
          <div className={data.children.length===0 ? `button-container-1`: `button-container`}>
            <button className="edit-button" onClick={handleEditChild}>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil" viewBox="0 0 16 16">
                ...
                <path fillRule="evenodd" d="M0 11.5v2a1 1 0 0 1 1 1h6a1 1 0 0 1 1-1H1a1 1 0 0 1-1-1v-2a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v6a1 1 0 0 1-1 1h-2a1 1 0 0 1-1-1v-6a1 1 0 0 1 1-1z"/>
              </svg>
            </button>
    ...
    <div className="hr">
    {childrenLength === undefined || childrenLength === 0 || childrenLength === 1 ? <></>:<hr style={{ width: `${contentLineWidth}px` }}></hr>}
    </div>
    </div>
    <div className={`add-card`}>
    {data.children.length === 0 && isAddingCard ? (
        <InputForm onSubmit={handleAddChildSubmit} onClose={handleCancelCard}/>
      ) : <></>}
      </div>
    {data.children && (
      <div className='child'>
        <div className="children" >
          {data.children.map((child, index) => (
            <div key={index} className="child-container"ref={contentLineRef}>
              <Card data={child} onDelete={onDelete} onAddChild={onAddChild} level={level +1} childrenLength={child.children.length}/>
            </div>
          ))}
        </div>
    ...
    {choice ? <div className="modal-container">
